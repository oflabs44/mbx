...
# write_cmd receives the new refresh token on stdin. Use $(cat) to splice the
# value into the assignment, drop op's stdin with </dev/null so op doesn't try
# to parse it as JSON. TOML literal strings (single-quoted) keep $ and " intact.
backend.auth.refresh-token.write_cmd = 'REPLACE  # e.g. op item edit mbx-{{name}} "refresh-token=$(cat)" </dev/null'
`
